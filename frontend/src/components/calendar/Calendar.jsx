  ...
  return (
    <div className="bg-white rounded shadow p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <h2 className="text-xl font-semibold cursor-default">{monthLabel}</h2>
      ...
      <div className="grid grid-cols-7 gap-2 text-center">
        {DAYS.map((day) => (
          <div key={day} className="font-semibold text-gray-600 cursor-default">
            {day}
          </div>
        ))}
        ...
        {cells.map((day, idx) => (
          ...
          >
            {day && <span className="text-sm cursor-default">{day}</span>}
          </div>
        ))}
      </div>
    </div>
